...
public class CommandHandler {
    ...
    public static void attemptCommandTriggers (IMessage message) {
        ...
        if (command == null) {
            DiscordUtilities.sendMessage(message.getChannel(), "No command was found by that name. " + key);
            return;
        }

        if (!command.isValidUsage(message)) {

            DiscordUtilities.sendPrivateMessage(message.getAuthor(), "You do not have permission to use the " + key + " command. Please try again, or look into getting elevated permissions.");
            return;
        }
